...
void ScorpioInterface::MoveEndEffectorTo(double x, double y, double z,  double quat_x, double quat_y, double quat_z, double quat_w) {
    std::cout << "-------------------------------" << std::endl;
    std::cout << "Scorpio Move to pos: " << x << ", " << y << ", " << z << std::endl;
    std::cout << "Scorpio Move to ori: " << quat_x << ", " << quat_y << ", " << quat_z << "," << quat_w << std::endl;
    std::cout << "-------------------------------" << std::endl;
    if (sp_->phase_copy == GRASPING_TEST_PHASE::HOLD_PH && !(sp_->is_opening) && !(sp_->is_closing) || sp_->is_holding) {
        ...
        des_pos << x, y, z;
        des_ori << quat_x, quat_y, quat_z, quat_w;
